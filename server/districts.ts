...
/**
 * Bir ilin veya ilçenin sınır kutusu. Harita bu alana yakınlaşmak için kullanır.
 * İlçe verilmezse ilin tamamını kapsayan kutu döner.
 */
export async function boundsFor(
  province: string,
  district?: string | null,
): Promise<{ minLat: number; maxLat: number; minLng: number; maxLng: number } | null> {
  const districts = await loadDistricts();
  const kapsam = districts.filter(
    (d) => d.province === province && (!district || d.name === district),
  );
  if (!kapsam.length) return null;
  return kapsam.reduce(
    (acc, d) => ({
      minLat: Math.min(acc.minLat, d.minLat),
      maxLat: Math.max(acc.maxLat, d.maxLat),
      minLng: Math.min(acc.minLng, d.minLng),
      maxLng: Math.max(acc.maxLng, d.maxLng),
    }),
